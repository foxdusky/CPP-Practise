...
void write_fl() {
	...
	cin >> wr.number;
	wr.length = read_i("Введите длину рулона: ");
	wr.width = read_i("Введите ширину рулона: ");
	...
	while (c) {
		...
		wr.expiration_date = read_i("Введите срок годности полотна (сколько лет может храниться?): ");
		cin.ignore();
		cout << "Введите плотность полотна : ";
		cin >> wr.density;
		wr.cost = read_fl("Введите стоимость за рулон: ");
...
int main()
{
	...
	while (true) {
		int x;
		system("cls");
		cout << "1. Запись данных в файл " << endl << "2. Вывод данных из файла" << endl << "3. Выйти из программы" << endl << "------------------> ";
		cin.ignore();
		cin >> x;
		switch (x) {
			case 1:
